...
<div class="flex items-end justify-between mb-stack-lg">
    ...
    <button onclick="openCreateUserModal(1)" class="px-5 py-2.5 bg-primary text-white rounded-full font-semibold hover:bg-primary-dark transition-all flex items-center gap-2">
...
<form method="get" action="admin.php" class="mb-stack-md">
    ...
    <div class="flex gap-3">
        ...
        <?php if (!empty($busqueda)): ?>
            <a href="admin.php?accion=listar_consumidores<?= ($mostrandoInactivos ?? false) ? '&inactivos=1' : '' ?>" class="px-4 py-2.5 rounded-full border border-outline-variant text-muted hover:bg-surface-container-low transition-colors flex items-center gap-1">
                <span class="material-symbols-outlined text-[18px]">close</span>
                Limpiar
            </a>
        <?php endif; ?>
        <a href="admin.php?accion=listar_consumidores<?= ($mostrandoInactivos ?? false) ? '' : '&inactivos=1' ?>"
                        ...
                        <tr class="group hover:bg-surface transition-colors">
                            ...
                            <td class="px-6 py-stack-md text-center">
                                <div class="flex items-center justify-center gap-1.5">
                                    ...
                                    <button onclick='openEditUserModal(<?= json_encode($c, JSON_HEX_TAG | JSON_HEX_AMP) ?>)'
                                        class="w-8 h-8 flex items-center justify-center rounded-lg bg-surface-container text-secondary hover:bg-primary hover:text-white transition-all"
                                        title="Editar usuario">
                                        <span class="material-symbols-outlined text-[18px]">edit</span>
                                    </button>
                                    <?php if (($c['estado'] ?? 'activo') === 'activo'): ?>
                                        <button onclick="openDeleteUserModal(<?= (int)$c['id'] ?>, '<?= htmlspecialchars(($c['nombre'] ?? '') . ' ' . ($c['apellido'] ?? ''), ENT_QUOTES, 'UTF-8') ?>')"
                                    ...
                                    <?php else: ?>
                                        <button onclick="openActivateUserModal(<?= (int)$c['id'] ?>, '<?= htmlspecialchars(($c['nombre'] ?? '') . ' ' . ($c['apellido'] ?? ''), ENT_QUOTES, 'UTF-8') ?>')"
